...
export default function Unpacker() {
  ...
  const processUnpack = async () => {
    if (jsonFiles.length === 0 || sheetFiles.length === 0) return;
    setLoading(true);
    setError(null);
    try {
      await unpackAtlas(jsonFiles, sheetFiles);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
      ...
      <p className="text-sm text-gray-500 mb-6">Upload JSON files and all corresponding sprite sheet images to extract the original images.</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
            <FileJson className="w-4 h-4" /> 1. Upload JSON files
          </label>
          <div className="flex items-center justify-center w-full">
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4 pointer-events-none">
                {jsonFiles.length > 0 ? (
                  <div className="flex flex-col items-center">
                    <span className="font-medium text-blue-600 truncate max-w-[200px]">{jsonFiles.length} JSONs selected</span>
                    <button 
                      onClick={(e) => { e.preventDefault(); setJsonFiles([]); }} 
              ...
              <input type="file" multiple className="hidden" accept=".json" onChange={handleJsonChange} />
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center justify-center w-full">
            ...
            >
              ...
              <input type="file" className="hidden" accept="image/png,image/jpeg,image/webp" multiple onChange={handleSheetsChange} />
            </label>
          </div>
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-gray-100 flex justify-end">
        <button 
          ...
          disabled={loading || jsonFiles.length === 0 || sheetFiles.length === 0}
